...
export const mountCalendar = (root, payload, now, options) => {
  ...
  const applyScrollTarget = () => {
    ...
    // ---- PROTOTYPE ONLY (prototype/month-landing) -----------------------
    // The three landing rules under test. `A` is what main ships today.
    const variant = new URL(doc.defaultView.location.href).searchParams.get("variant") ?? "A";
    /** Views that lay their days out across the top rather than down the page. */
    const horizontal = state.view === "month" || state.view === "week";
    if (variant === "B" && horizontal) {
      scrollTarget = null;
      if (typeof surface.scrollIntoView === "function") surface.scrollIntoView({ block: "start" });
      return;
    }
    if (variant === "C" && state.view === "month") {
      scrollTarget = null;
      return;
    }
    // ---------------------------------------------------------------------
