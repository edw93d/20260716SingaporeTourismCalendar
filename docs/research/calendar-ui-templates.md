> ✅ **Current as the record of why the web calendar has the shape it has.** Part 2's verdict — the
> month grid cannot be the reading surface, and survives only as a density navigator you drill *from*
> — is quoted in [ADR-0009](../adr/0009-web-calendar-four-switchable-views.md)'s *Context* as one of
> the two findings that framed it, and is what the built product does.
> [ADR-0014](../adr/0014-web-calendar-presentation-refinements-uiux-round-1.md) refines that shape
> rather than reversing it.
>
> **Part 3's component shortlist is a decision record, not a live one.** Its "Open questions"
> §1 — the AGPLv3-or-pay licensing fork it called the biggest — was answered by taking neither arm:
> the web calendar was hand-rolled in vanilla JS in [`site/`](../../site/), and no calendar library is
> a dependency. Prices and licence terms in §3 were verified on 16 Jul 2026 and are not maintained.
> Its §3.8 warning — that shadcn/ui's `Calendar` is a date picker and cannot render this dataset — is
> the kind of wrong turn worth leaving on the record.
