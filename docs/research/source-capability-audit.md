> ⚠️ **Superseded *in part* by [`v2-source-acquisition.md`](v2-source-acquisition.md) on 04 Aug 2026
> — kept for the record, because it records what was read on 16 Jul 2026, and because part of it is
> still load-bearing.**
>
> **Superseded as a source survey.** Four sources here; #113 covers twenty-one, and its route
> vocabulary (`api` / `feed` / `http-scrape` / `browser` / `none`) replaces this document's
> API/rendering/anti-bot columns. Its Ticketmaster verdict now lives as a hard stop in
> [`docs/source-register.md`](../source-register.md); its legal reads are superseded by
> [ADR-0021](../adr/0021-reading-sources-that-forbid-it.md).
>
> **Not superseded as the field-level record of the three production v1 adapters.** #113 established
> *whether* each of twenty-one sources can be read; it did not re-derive *which bytes to parse* for
> `suntec`, `mbccs` and `scc`, which are the adapters running today. Still current and not recorded
> anywhere else: the Google Calendar export link carrying a full UTC interval on 154/154 Suntec
> events, and the `<time datetime>` trap that silently drops the clock time (§1); the MBCCS
> `berthingDateTime` / `unberthingDateTime` field map (§3); the SCC column formats and the
> whitespace-only ship/line delimiter (§4). [ADR-0003](../adr/0003-store-utc-instants.md) cites this
> document's end-time table as its reason for retiring the inclusive-end-date rule.
>
> **§3's security finding was acted on as precedent, not as a route in.** The MBCCS Basic-auth
> credential in a public JS bundle is described and located here but never reproduced;
> [`changi-arrivals.md`](changi-arrivals.md) §*Ethics note* names this handling as the pattern it
> follows, and ADR-0021 §7 later made it a rule.
