> ⚠️ **Superseded by [ADR-0021](../adr/0021-reading-sources-that-forbid-it.md) and
> [`docs/source-register.md`](../source-register.md) on 05 Aug 2026 — kept for the record, because it
> records what was read on 16 Jul 2026.** Four sources here; the register carries twenty. Its
> per-source verdicts are stale, and its recommendation 1 (drop Ticketmaster) survives as a hard stop
> in the register.
>
> **One correction, because the record is the point.** ADR-0021's *Context* describes v1's audit as
> having "checked `robots.txt` and stopped there". **That is not what this document did.** It quotes
> all four sources' terms verbatim, reasons from *Global Yellow Pages v Promedia* [2017] SGCA 28, and
> flags by name two clauses the register now carries as operative — SCC's "internal, non-commercial"
> limb (§4) and MBCCS's anti-mirroring clause (§3). What is accurate is the *other* half of the
> register's sentence: the audit "left no record", because this branch never merged. The finding was
> made and then lost, which is why #113 re-derived SCC's clause on 04 Aug and marked it **new**.
>
> **One genuine miss, and it is Suntec.** §1 quotes only the `meta-search` half of Suntec's
> automation clause and reads it as narrow SEO tooling. The same sentence continues *"or cause to be
> sent, any automated queries of any sort"* — see
> [`v2-source-acquisition.md`](v2-source-acquisition.md) §2, which quotes it whole and calls it
> "materially weaker footing than v1 recorded". Reading a clause to its comma is the failure mode
> this document is worth keeping to illustrate.
