> ⚠️ **Superseded by [`v2-source-acquisition.md`](v2-source-acquisition.md) on 04 Aug 2026 — kept for
> the record, because it records what was read on 16 Jul 2026.** This ranked six candidate sources;
> #113 surveyed twenty-one and settled a route for each. Its SISTIC section — "licensing posture
> **unresolved**", terms unreadable without JS — was resolved by
> [`sistic-capability-legal-audit.md`](sistic-capability-legal-audit.md) on 05 Aug 2026, which read
> the terms and found the one genuine API on the whole list.
>
> **Two findings here outlived the survey.** The TIH death certificate (§*Headline*) — NXDOMAIN plus
> the dead `go.gov.sg` shortlink — is still the primary evidence, carried forward as row 11 of #113;
> its warning that search engines and LLMs still describe TIH's events API in the present tense
> remains live. And the 15 Jul 2026 SCC HarbourFront relocation was caught mid-move, one day after it
> happened.
