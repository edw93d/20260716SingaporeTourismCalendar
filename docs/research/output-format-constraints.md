> ✅ **Current as research.** Its findings about RFC 5545 and the three clients still stand and are
> still cited: [ADR-0001](../adr/0001-two-types-one-projection.md) and
> [ADR-0008](../adr/0008-ical-feed-shape-two-type-split-feeds.md) both rest on §3's result that
> `CATEGORIES` survives on 1 of 3 clients — that is *why* ADR-0008 splits the feed by type rather
> than letting subscribers filter one firehose. §9's reduced-projection position is the shape the
> serializer was built in.
>
> **Two of its recommendations were later overturned by decision, not by better evidence — read §4,
> §8.4 and §10 against these.**
>
> 1. **Inclusive end dates: retired.** §4 and §8.4 recommend storing the inclusive end and letting the
>    serializer add a day. [ADR-0003](../adr/0003-store-utc-instants.md) (16 Jul 2026) retires that
>    rule and stores UTC instants. The recommendation was written while Ticketmaster SG — the one
>    date-only source — was still seeded; the legal audit
>    ([`legal-tos-audit.md`](legal-tos-audit.md)) dropped it, and all three surviving sources publish
>    true end times, so the +1 would have discarded real clock times.
> 2. **`SEQUENCE`: stored, not serialized.** §10's envelope emits it. ADR-0008 §5, amended 20 Jul 2026
>    ([#33](https://github.com/edw93d/20260716SingaporeTourismCalendar/issues/33)), does not — clients
>    consult it for iTIP scheduling messages, and these are `METHOD`-less published feeds.
>
> §6's closing observation — that "daily" is honoured by the pipeline and not by a subscriber's
> calendar, so the web calendar is the only surface where it is true — is the argument ADR-0008 later
> used to kill the firehose feed.
